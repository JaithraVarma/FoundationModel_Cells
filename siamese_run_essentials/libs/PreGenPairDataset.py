...
class PreGenPairDataset(Dataset):
    ...
    def __init__(self, path, shuffle_pairs=True, augment=False, num_workers=16):
        ...
        self.data["img1_class"] = self.data["img1"].apply(lambda x: x.split("/")[-2])
        self.data["img2_class"] = self.data["img2"].apply(lambda x: x.split("/")[-2])
        self.data["pat1"] = self.data["img1_class"].apply(lambda x: x.split("_")[1])
        self.data["pat2"] = self.data["img2_class"].apply(lambda x: x.split("_")[1])

        self.data = self.data[(self.data["pat1"].isin(['9346', '9633', '9502'])) & (self.data["pat2"].isin(['9346', '9633', '9502']))]
    ...
    def __getitem__(self, idx):
        ...
        image_path1 = idx_data['img1']
        image_path2 = idx_data['img2']
        ...
        label = int(idx_data['label'])

        label = torch.FloatTensor([label == 1])

        return (image1, image2), label, (image_path1, image_path2)
